...
const isCorrectGuess = (secretNumber: number | null, n: number) => n && secretNumber && n === secretNumber;

const numberColor = (guesses: number[], secretNumber: number | null, n: number) => {
...
const MaybeTryAgainButton: React.FC<TryAgainButtonProps> = (props: TryAgainButtonProps) => {
    const {guessedCorrectly, newSecretNumberRequested} = props
    if (guessedCorrectly)
        return (
            <button className="button is-outlined is-hovered" onClick={newSecretNumberRequested}>
                Try a new number
            </button>
        );
    else
...
const GuessButtons: React.FC<GuessButtonsProps> = (props: GuessButtonsProps) => {
    const {guessedCorrectly, guesses, secretNumber, numbers, playerGuessed} = props;
    return <>{numbers.map(n => <GuessButton key={n} n={n} guessedCorrectly={guessedCorrectly} guesses={guesses}
                                            playerGuessed={playerGuessed} secretNumber={secretNumber}/>)}</>;
};
...
const App = () => {
    ...
    const [secretNumber, setSecretNumber] = useState<number | null>(Math.floor(Math.random() * 6 + 1));
            ...
            <div className="hero-body">
                <div className="container">
                    <h1 className="title">
                        ...
                        <MaybeTryAgainButton guessedCorrectly={guessedCorrectly}
